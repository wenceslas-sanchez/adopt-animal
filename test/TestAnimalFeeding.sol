...
contract TestAnimalFeeding {
    ...
    AnimalFeeding animalFeeding =
        AnimalFeeding(DeployedAddresses.AnimalFeeding());
    ...
    function testSetCoolDownNotWork() public {
        (bool r, ) = address(this).call(
            abi.encodePacked(this._setCoolDown.selector)
        );
        Assert.isFalse(
            r,
            "Should not change cooldown state variable if not called by owner."
        );
    }
    ...
    function testSetFoodCostNotWork() public {
        (bool r, ) = address(this).call(
            abi.encodePacked(this._setFoodCost.selector)
        );
        Assert.isFalse(
            r,
            "Should not change food cost state variable if not called by owner."
        );
    }
